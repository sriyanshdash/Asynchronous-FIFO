//=============================================================================
// File        : asynchronous_fifo_tb_copilot.sv
// Description : Complete Asynchronous FIFO Testbench — single-file consolidation.
//               Contains ALL testbench components and 28 test cases.
//
// Usage       : xrun asynchronous_fifo_rtl_copilot.sv \
//                    asynchronous_fifo_tb_copilot.sv \
//                    +TEST_NAME=all +SVSEED=1234 +define+DUMP_ON
//
...
interface fifo_if #(parameter FIFO_WIDTH = 64) (
    ...
    modport dut_mp (
        ...
        output fifo_full, data_out, fifo_empty
    );

    modport wr_tb_mp (
        input  wrclk, fifo_full,
        output wrst_n, wr_en, data_in
    );

    modport rd_tb_mp (
        input  rdclk, data_out, fifo_empty,
        output rrst_n, rd_en
    );
...
//=============================================================================
// 9. TEST CLASSES — Normal Operation Tests
//=============================================================================
...
//=============================================================================
// 10. TEST CLASSES — Negative / Edge-Case Tests
//=============================================================================
...
class test_overflow_underflow #(
    ...
    task test_overflow();
        ...
        repeat (6) @(posedge vif.wrclk);
        if (!vif.fifo_full) begin
            $display("[TEST_OVF_UNF] FAIL: FIFO not full after %0d writes", FIFO_DEPTH);
            local_fail++; return;
        end
...
class test_back_to_back_overflow #(
    ...
    virtual task run();
        ...
        repeat (10) begin
            @(posedge vif.wrclk); #1; vif.data_in = vif.data_in - 1;
        end
        vif.wr_en = 1'b0; vif.data_in = '0;
        read_n(FIFO_DEPTH); wait_drain(5000);
        repeat (6) @(posedge vif.rdclk);
